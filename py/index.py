...
async def create(request: Request) -> Response:
    is_color = request.message == "color"
    print(is_color)
    wimg = Image.open(io.BytesIO(request.event.imageList[0]))
    bimg = Image.open(io.BytesIO(request.event.imageList[1]))
    if is_color:
        car = await color_car(wimg, bimg)
    else:
        car = await gray_car(wimg, bimg)

    return Response(image=BytesIOToBytes(car))
